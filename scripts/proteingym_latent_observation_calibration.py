#!/usr/bin/env python3
"""Run EXP-003: ProteinGym raw-scale latent observation calibration panel."""
...
from adaptenv import AlignmentProfile, LandscapeConfig  # noqa: E402
...
class ProteinGymEmpiricalLandscape:
    """Expose a ProteinGym assay on an arbitrary coordinate system to adaptenv."""
...
def select_assay_panel(reference_df: pd.DataFrame, panel_cfg: dict[str, Any]) -> pd.DataFrame:
    ...
    filtered = filtered[filtered["DMS_number_multiple_mutants"].fillna(0).astype(int) >= min_multiple_mutants]
...
def fit_mavenn_model(
    *,
    ...
    try:
        ...
        raise RuntimeError(
            "mavenn is required for EXP-003. Ensure the experiment overlay is applied."
        ) from exc
...
def summarize_branch(
    *,
    ...
    return {
        "branch": branch_name,
        "fitted_parameters": to_builtin(calibration_result.fitted_parameters),
        "validation": to_builtin(calibration_result.validation),
        "objective_terms": to_builtin(calibration_result.objective_terms),
    }
...
def main() -> None:
    ...
    calibration_cfg = dict(config["calibration"])
    ...
    shared_raw_landscapes: list[ProteinGymEmpiricalLandscape] = []
    shared_latent_landscapes: list[ProteinGymEmpiricalLandscape] = []
    alignment_paths: list[Path] = []
    wildtypes: list[str] = []
    per_assay_payload: list[dict[str, Any]] = []

    calibration_max_mutation_count = int(calibration_cfg["calibration_max_mutation_count"])
    ...
    for index, row in enumerate(panel_df.itertuples(index=False)):
        ...
        calibration_subset = subset[subset["mutation_count"] <= calibration_max_mutation_count].copy()
        ...
        raw_summary = summarize_empirical_landscape(
            raw_landscape,
            kind="functional",
            alignment_profile=alignment_path,
            wildtype=wildtype_sequence,
            options=calibration_options_from_config(calibration_cfg["shared_raw"]),
            index=index,
        )
        latent_summary = summarize_empirical_landscape(
            latent_landscape,
            kind="functional",
            alignment_profile=alignment_path,
            wildtype=wildtype_sequence,
            options=calibration_options_from_config(calibration_cfg["shared_latent"]),
            index=index,
        )
        ...
        per_assay_payload.append(
            {
                ...
                "mavenn": to_builtin(mavenn_metrics),
                ...
                "raw_effect_summary": to_builtin(raw_summary.effect_summary),
                "latent_effect_summary": to_builtin(latent_summary.effect_summary),
            }
        )

    shared_raw_result = calibrate_synthetic_landscape(
        functional_landscapes=shared_raw_landscapes,
        functional_alignment_profiles=alignment_paths,
        functional_wildtypes=wildtypes,
        base_config=LandscapeConfig(**base_config),
        options=calibration_options_from_config(calibration_cfg["shared_raw"]),
    )
    shared_latent_result = calibrate_synthetic_landscape(
        functional_landscapes=shared_latent_landscapes,
        functional_alignment_profiles=alignment_paths,
        functional_wildtypes=wildtypes,
        base_config=LandscapeConfig(**base_config),
        options=calibration_options_from_config(calibration_cfg["shared_latent"]),
    ...
    per_assay_latent_rows: list[dict[str, Any]] = []
    for assay_payload, latent_landscape, alignment_path, wildtype_sequence in zip(
        per_assay_payload,
        shared_latent_landscapes,
        alignment_paths,
        wildtypes,
    ):
        assay_result = calibrate_synthetic_landscape(
            functional_landscapes=[latent_landscape],
            functional_alignment_profiles=[alignment_path],
            functional_wildtypes=[wildtype_sequence],
            base_config=LandscapeConfig(**base_config),
            options=calibration_options_from_config(calibration_cfg["per_assay_latent"]),
        )
        per_assay_latent_rows.append(
            {
                "dms_id": assay_payload["dms_id"],
                "fitted_parameters": to_builtin(assay_result.fitted_parameters),
                "validation": to_builtin(assay_result.validation),
                "objective_terms": to_builtin(assay_result.objective_terms),
            }
        )
    ...
    per_assay_latent_fits_path = output_dir / "per_assay_latent_fits.csv"
    pd.DataFrame(
        [
            {
                "dms_id": row["dms_id"],
                ...
                **{
                    f"fitted__{key}": value
                    for key, value in row["fitted_parameters"].items()
                },
            }
            for row in per_assay_latent_rows
        ]
    ).to_csv(per_assay_latent_fits_path, index=False)
    ...
    summary_payload = {
        ...
        "branches": {
            "shared_raw": summarize_branch(
                branch_name="shared_raw",
                calibration_result=shared_raw_result,
            ),
            "shared_latent": summarize_branch(
                branch_name="shared_latent",
                calibration_result=shared_latent_result,
            ),
        },
        "per_assay_latent": per_assay_latent_rows,
